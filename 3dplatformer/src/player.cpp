...
void player::update() {
	...
	if (thebanana::debug::control) {
		...
		if (this->m_game->get_input_manager()->get_device_type(0) == thebanana::input_manager::device_type::keyboard) {
			std::vector<thebanana::input_manager::device::button> btns = this->m_game->get_input_manager()->get_device_buttons(0);
			if (btns[DIK_W].held) {
		...
		else if (this->m_game->get_input_manager()->get_device_type(0) == thebanana::input_manager::device_type::controller) {
			constexpr float deadzone = 0.05f;
			glm::vec2 left_stick = ((thebanana::controller*)this->m_game->get_input_manager()->get_device(0))->get_joysticks().left;
